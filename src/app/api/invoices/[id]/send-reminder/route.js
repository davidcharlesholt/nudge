...
export async function POST(_req, context) {
  try {
    ...
    if (invoice.status !== "sent" && invoice.status !== "overdue") {
      return Response.json(
        { ok: false, error: "Reminders can only be sent for invoices with status 'sent' or 'overdue'" },
        { status: 400 }
      );
    }
    ...
    const reminderTemplates = schedule.templates.filter((t) => t.offset !== null);
    ...
    if (nextReminderIndex >= reminderTemplates.length) {
      return Response.json(
        { ok: false, error: "All reminders for this invoice have already been sent" },
        { status: 400 }
      );
    }
    ...
    const reminderTemplate = invoiceTemplates.find((t) => t.id === nextReminderTemplateId);

    if (!reminderTemplate) {
      return Response.json(
        { ok: false, error: `Reminder template '${nextReminderTemplateId}' not found in invoice templates` },
        { status: 400 }
      );
    }
    ...
    if (remindersSent.includes(nextReminderTemplateId)) {
      return Response.json(
        { ok: false, error: `Reminder '${nextReminderTemplateId}' has already been sent` },
        { status: 400 }
      );
    }
